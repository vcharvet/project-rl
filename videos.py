...
def choose_sample(segment1, segment2):
    ...
    axes = AxesSequence()
    axes2 = AxesSequence
    for i, ax in zip(range(segment1.shape[0]), axes):
        ax.imshow(segment1[i, :, :, :])
        ax.set_title('First Segment - {}th frame'.format(i))
        # ax[1].imshow(segment2[i, :, :, :])
        # ax[1].set_title('Second Segment - {}th frame'.format(i))
    for i, ax in zip(range(segment2.shape[0]), axes):
        ax.imshow(segment2[i, :, :, :])
        ax.set_title('Second Segment - {}th frame'.format(i))
    axes.show()
